...
RESULTS_PATH  = "results/sweep_dwell_results.csv"

DWELL_THRESHOLDS = [15, 30, 45, 60]
CONFIGS = [
...
def run_clip(video_path, roi, use_dwell, use_motion, dwell_threshold):
...
def main():
    ...
    for dwell_thresh in DWELL_THRESHOLDS:
        print(f"\n=== Dwell threshold: {dwell_thresh}s ===")
        ...
        for config in CONFIGS:
            ...
            for clip in clips:
                ...
                alerts  = run_clip(video_path, roi, config["use_dwell"],
                                   config["use_motion"], dwell_thresh)
